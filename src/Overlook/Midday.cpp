...
void Midday::InitEA() {
	...
	/*if (!(IsDemo() || IsTesting() || IsOptimization()))
	{
	   if (7.0 * (5.0 * (3.0 * AccountNumber() + 4.0) + 7.0) != LicenseKey) {
	      ...
	      ls_8 = "Ïîëó÷èòü ëèöåíçèîííûé êëþ÷ ìîæíî íà";
	      ls_16 = "www.peregruzok.net/index.php/licensekey.html";
	      Print(ls_0);
	      Print(ls_8);
	      Print(ls_16);
	      MessageBox(StringConcatenate(ls_0,
	         "\n", ls_8,
	      "\n", ls_16), "Îøèáêà", MB_ICONHAND);
	      gi_400 = true;
	      Sleep(86400000);
	   }
	}*/
...
void Midday::StartEA(int pos) {
	...
	if (l_ticket_72 == 0 && gi_224) {
		...
		if ((TimeHour(TimeCurrent()) >= timestart1 && TimeHour(TimeCurrent()) < timestop1) || (TimeHour(TimeCurrent()) >= timestart2 && TimeHour(TimeCurrent()) < timestop2))
			OrderSend(Symbol(), OP_BUY, g_lots_232, Ask, 3, l_price_16, l_price_32, "", magic, 0, Blue);
	}
	
	if (l_ticket_76 == 0 && gi_224) {
		if (l_ticket_84 != 0)
		...
		if ((TimeHour(TimeCurrent()) >= timestart1 && TimeHour(TimeCurrent()) < timestop1) || (TimeHour(TimeCurrent()) >= timestart2 && TimeHour(TimeCurrent()) < timestop2))
			OrderSend(Symbol(), OP_SELL, g_lots_232, Bid, 3, l_price_8, l_price_24, "", magic, 0, Red);
	}
...
double Midday::getDayProfit(int ai_0, int ai_4) {
	int l_year_8;
	int l_month_12;
	int l_day_16;
	int l_datetime_20;
...
double Midday::getDayProfitall(int ai_0, int ai_4) {
	int l_year_8;
	int l_month_12;
	int l_day_16;
	int l_datetime_20;
...
int Midday::LevelMM(double ad_0) {
	int ad_1 = NormalizeDouble(ad_0 / g_lots_232, 0);
...
int Midday::ProfitMM(double ad_0) {
	int ad_1 = NormalizeDouble(ad_0 / g_lots_232, 0);
