...
void DQNDoubleAgent::SetArguments(const VectorMap<String, Value>& args) {
	
}

void DQNDoubleAgent::Init() {
...
bool DQNDoubleAgent::Process(const SlotProcessAttributes& attr) {
	...
	if (attr.tf_id == 0 && attr.sym_id == 0) {
		...
		LOG(Format("DQNDoubleAgent::Process sym=%d tf=%d pos=%d", attr.sym_id, attr.tf_id, attr.GetCounted()));
		Cout() << Format("DQNDoubleAgent::Process sym=%d tf=%d pos=%d", attr.sym_id, attr.tf_id, attr.GetCounted()) << "\n";
...
void DQNDoubleAgent::Forward(const SlotProcessAttributes& attr) {
...
void DQNDoubleAgent::Backward(const SlotProcessAttributes& attr) {
...
void MonaDoubleAgent::Init() {
	...
	total = sym_count * tf_count *  max_shift + sym_count + 1 + 1 + 1 + 1;
...
void MonaDoubleAgent::Backward(const SlotProcessAttributes& attr) {
