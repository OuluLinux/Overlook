...
void GraphCtrl::Paint(Draw& draw) {
	...
	for(int i = 0; i < src.GetCount(); i++) {
		MetaVar& var = src[i];
		if (!var.Is()) continue;
		Container& cont = *var.Get<Container>();
		BarData* bardata = dynamic_cast<BarData*>(&cont);
		if (bardata) {
			BarData& ts = dynamic_cast<BarData&>(cont);
			...
			int bufs = cont.GetBufferCount();
			//BarData* bdcont = dynamic_cast<BarData*>(&cont);
			//ASSERT(bdcont != 0);
			for(int j = 0; j < bufs; j++) {
				PaintContainerLine(w, cont, shift, i==0, j);
			}
		}
	}
    ...
    if (show_timevalue_tool) {
        ...
        if (latest_mouse_move_pt.y >= 0) {
	        ...
	        int pos = dt.GetCount(period) - count + (x - border) / div - shift;
	        if (pos >= 0 && pos < data_count) {
	            Time t = dt.GetTime(period, pos);
...
void GraphCtrl::DrawGrid(Draw& W, bool draw_vert_grid) {
	...
	BarData& pb = group->GetBarData();// GetValues(id, tf);
	MetaTime& dt = src[0]->GetTime();
    ...
    for(int i = gridw - 1; i >= gridstep; i-=gridstep ) {
        ...
        Time time = dt.GetTime(period, pos);
...
void GraphCtrl::PaintCandlesticks(Draw& W, BarData& values) {
	DrawGrid(W, true);
...
void GraphCtrl::PaintContainerLine(Draw& W, Container& cont, int shift, bool draw_border, int buffer) {
...
void GraphCtrl::DrawLines(Draw& d, Container& cont) {
	if (cont.IsContainerSeparateWindow()) {
		Rect r = GetGraphCtrlRect();
		double diff = hi - lo;
		int h = r.GetHeight();
		int w = r.GetWidth();
		...
		String text;
		for(int i = 0; i < cont.GetContainerLevelCount(); i++) {
			double value = cont.GetContainerLevelValue(i);
			int type = cont.GetContainerLevelType(i);
			int line_width = cont.GetContainerLevelLineWidth(i);
...
int GraphCtrl::GetCount() {
	if (!group) return 0;
	int c = group->GetBarData().GetBufferDataCount();
    return c;
}
