...
bool GridAdvisor::TrainingIterator() {
...
bool GridAdvisor::TrainingInspect() {
	bool success = false;
...
void GridAdvisor::TrainingCtrl::Paint(Draw& w) {
	...
	GridAdvisor* ea = dynamic_cast<GridAdvisor*>(&*job->core);
	ASSERT(ea);
	DrawVectorPolyline(id, sz, ea->training_pts, polyline);
...
/*double GridAdvisor::TestGrid() {
	...
	double result = 1.0;
	double spread = GetDataBridge()->GetSpread();
	if (spread == 0.0) spread = GetDataBridge()->GetPoint() * 4;
