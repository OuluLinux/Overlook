...
float FixedSimBroker::RealtimeAsk(const Snapshot& snap, int sym_id) const PARALLEL {
	return snap.open[sym_id];
}

double FixedSimBroker::GetCloseProfit(int sym_id, const FixedOrder& o, const Snapshot& snap) const PARALLEL {
...
void FixedSimBroker::OrderSend(int sym_id, int type, float volume, float price) PARALLEL {
	ASSERT(sym_id >= 0 && sym_id < SYM_COUNT);
...
void FixedSimBroker::OrderClose(int sym_id, double lots, FixedOrder& order, const Snapshot& snap) PARALLEL {
	if (!order.is_open)
		return;
...
double FixedSimBroker::GetMargin(const Snapshot& snap, int sym_id, double volume) PARALLEL {
	ASSERT(leverage > 0);
...
void FixedSimBroker::Cycle(const Snapshot& snap) PARALLEL {
	...
	for(int i = 0; i < SYM_COUNT; i++) {
		...
		buy_lots[i]  = ((int64)(sym_buy_lots  / 0.01)) * 0.01;
		sell_lots[i] = ((int64)(sym_sell_lots / 0.01)) * 0.01;
	}
