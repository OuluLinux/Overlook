...
namespace Util {
    ...
    inline HRESULT setEventHandlers(_In_ IToastNotification* notification, _In_ std::shared_ptr<NotificationBase> eventHandler, _In_ INT64 expirationTime) {
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            if (arguments && *arguments) {
                                eventHandler->NotificationActivated((int)wcstol(arguments, NULL, 10));
                                return S_OK;
                            }
                        }
                    }
                    eventHandler->NotificationActivated();
                    return S_OK;
                }).Get(), &activatedToken);
            ...
            hr = notification->add_Dismissed(Callback < Implements < RuntimeClassFlags<ClassicCom>,
                     ...
                     [eventHandler, expirationTime](IToastNotification*, IToastDismissedEventArgs* e)
                 {
                     ...
                     if (SUCCEEDED(e->get_Reason(&reason)))
                     {
                         ...
                         eventHandler->NotificationDismissed(static_cast<WinToastDismissalReason>(reason));
                     }
                     return S_OK;
                 }).Get(), &dismissedToken);
            if (SUCCEEDED(hr)) {
                ...
                {
                    eventHandler->NotificationFailed();
                    return S_OK;
                }).Get(), &failedToken);
            }
        }
...
bool PushNotification::WinToast::isSupportingModernFeatures() {
...
INT64 WinToast::showToast(_In_ const WinToastTemplate& toast, _In_  NotificationBase* handler, _Out_ WinToastError* error)  {
    setError(error, WinToastError::NoError);
    INT64 id = -1;
    if (!isInitialized()) {
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        if (SUCCEEDED(hr)) {
                            ...
                            if (SUCCEEDED(hr)) {
                                ...
                                if (SUCCEEDED(hr)) {
                                    hr = Util::setEventHandlers(notification.Get(), std::shared_ptr<NotificationBase>(handler), expiration);
                                    if (FAILED(hr)) {
                                        setError(error, WinToastError::InvalidHandler);
                                    }
                                }
...
int PushToast(std::wstring appname, std::wstring msg, std::vector<std::wstring> actions, std::wstring imgpath, bool silent, void* base, int expiration_) {
    ...
    LPWSTR appName = (LPWSTR)appname.c_str(),
        appUserModelID = (LPWSTR)appname.c_str(),
        text = NULL,
        imagePath = NULL,
        attribute = L"default";
    
    INT64 expiration = expiration_;
   

    bool onlyCreateShortcut = false;
    WinToastTemplate::AudioOption audioOption =
		!silent ? WinToastTemplate::AudioOption::Default : WinToastTemplate::AudioOption::Silent;
    
    text = (LPWSTR)msg.c_str();
    
    if (imgpath.size())
        imagePath = (LPWSTR)imgpath.c_str();
